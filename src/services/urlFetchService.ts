...
export class UrlFetchService {
  ...
  private validateUrl(urlString: string): URL {
    ...
    if (!ALLOWED_PORTS.includes(port)) {
      throw new SecurityError(
        `허용되지 않은 포트입니다. 허용된 포트: ${ALLOWED_PORTS.join(", ")}`,
      );
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const request = client.request(options, (response) => {
        ...
        if (
          ...
          reject(
            new NetworkError(
              `HTTP 오류: ${response.statusCode} ${response.statusMessage}`,
            ),
          );
          return;
        }
        ...
        if (contentLength && parseInt(contentLength) > MAX_RESPONSE_SIZE) {
          reject(
            new ValidationError(
              `응답 크기가 너무 큽니다. 최대 ${MAX_RESPONSE_SIZE / 1024 / 1024}MB까지 허용됩니다.`,
            ),
          );
          return;
        }
        ...
        response.on("end", () => {
          ...
          logger.info(
            `URL에서 스키마 가져오기 완료: ${urlString} (크기: ${totalSize} bytes)`,
          );
          resolve(result);
        });
  ...
  private validateContent(content: Buffer, contentType: string): void {
    ...
    if (contentType.includes("xml")) {
      ...
      // XXE 공격 방지
      if (
        contentString.includes("<!ENTITY") &&
        contentString.includes("SYSTEM")
      ) {
        throw new SecurityError(
          "외부 엔티티 참조가 포함된 XML은 허용되지 않습니다.",
        );
      }
    }
