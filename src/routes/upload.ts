...
// 파일 업로드 엔드포인트
router.post('/file', upload.single('file'), asyncHandler(async (req: Request, res: Response) => {
  if (!req.file) {
...
// 파일 내용 다운로드
router.get('/file/:fileId/content', asyncHandler(async (req: Request, res: Response) => {
  const { fileId } = req.params;
