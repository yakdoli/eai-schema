...
import { Multiplicity, ParameterMode, } from '@eai/shared/types/sap';
...
export class ODataAdapter {
    ...
    async parseSchema(schema, options) {
    ...
    async parseEntityTypes(entityTypesData) {
        ...
        return entityTypesData.map((entityType) => ({
            ...
            navigationProperties: this.parseNavigationProperties(entityType.NavigationProperty || []),
            annotations: this.parseAnnotations(entityType.Annotation || []),
        }));
    }
    parseEntityKey(keyData) {
        ...
        const propertyRefs = Array.isArray(keyData.PropertyRef)
            ...
            : [keyData.PropertyRef];
        return {
            propertyRefs: propertyRefs.map((ref) => ({
                name: ref['@_Name'],
            })),
        };
    }
    parseProperties(propertiesData) {
    ...
    parseNavigationProperties(navPropsData) {
    ...
    async parseComplexTypes(complexTypesData) {
    ...
    async parseAssociations(associationsData) {
        ...
        return associationsData.map((association) => ({
            ...
            referentialConstraint: this.parseReferentialConstraint(association.ReferentialConstraint),
        }));
    }
    parseAssociationEnds(endsData) {
    ...
    parseReferentialConstraint(constraintData) {
        if (!constraintData)
            return undefined;
        return {
            principal: {
    ...
    parseEntitySets(entitySetsData) {
    ...
    parseFunctionImports(functionImportsData) {
    ...
    parseFunctionParameters(parametersData) {
    ...
    parseAnnotations(annotationsData) {
    ...
    async validate(schema) {
        ...
        for (const entitySet of schema.content.entitySets) {
            const entityType = schema.content.entityTypes.find((et) => `${schema.metadata.namespace}.${et.name}` === entitySet.entityType);
            if (!entityType) {
                errors.push({
    ...
    countElements(content) {
        return (content.entityTypes.length +
