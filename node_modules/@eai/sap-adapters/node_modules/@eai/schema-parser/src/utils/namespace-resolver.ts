...
export class NamespaceResolver {
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      if (typeof value === 'object' && value !== null) {
        if (Array.isArray(value)) {
          processedData[processedKey] = value.map(item => 
            this.processNamespaces(item, namespaces)
          );
        } else {
   ...
   * @returns 충돌 정보 배열
   */
  checkNamespaceConflicts(namespaces: NamespaceContext): Array<{prefix: string, conflictingUris: string[]}> {
    const conflicts: Array<{prefix: string, conflictingUris: string[]}> = [];
    const prefixUsage = new Map<string, string[]>();
  ...
  private currentContext?: NamespaceContext;
}