...
export enum SAPQualityOfService {
...
export class SOAPAdapter implements SAPAdapter<SAPSOAPSchema> {
  ...
  async parse(
  ...
  ): Promise<SAPParseResult<SAPSOAPSchema>> {
    ...
    try {
      ...
      // SAP SOAP 스키마로 확장
      const result: SAPSOAPSchema = {
        ...wsdlResult.schema,
        type: 'SAP_SOAP' as const,
        sapExtensions,
      };
  ...
  private async extractSAPExtensions(
    content: string,
    options: SAPParseOptions
  ): Promise<SAPSOAPExtensions> {
    const extensions: SAPSOAPExtensions = {};
  ...
  private parseQualityOfService(qos: string): SAPQualityOfService {
  ...
  private extractRoutingRules(content: string): SAPRoutingRule[] {
    const rules: SAPRoutingRule[] = [];
  ...
  private extractTransformationRules(content: string): SAPTransformationRule[] {
    const rules: SAPTransformationRule[] = [];
  ...
  private extractErrorHandling(content: string): SAPErrorHandling | undefined {
    const errorHandlingMatch = content.match(/<sap:ErrorHandling[^>]*strategy="([^"]*)"[^>]*>/i);
    if (!errorHandlingMatch) return undefined;

    const strategy = errorHandlingMatch[1] as SAPErrorHandling['strategy'];
  ...
  async validate(schema: SAPSOAPSchema): Promise<SAPValidationResult> {
  ...
  private countElements(schema: SAPSOAPSchema): number {
    const baseCount = (
      schema.content.services.length +
    ...
    const sapExtensionCount = (
      (schema.sapExtensions.routingRules?.length || 0) +
