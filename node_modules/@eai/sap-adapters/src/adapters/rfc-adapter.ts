...
export class RFCAdapter implements SAPAdapter<SAPRFCSchema> {
  ...
  async parse(
  ...
  ): Promise<SAPParseResult<SAPRFCSchema>> {
    ...
    try {
      ...
      let parsedContent: SAPRFCContent;
  ...
  private async parseXMLRFC(
    content: string,
    options: SAPParseOptions
  ): Promise<SAPRFCContent> {
  ...
  private async parseTextRFC(
    ...
    let functionGroup = '';
    let shortText = '';
    const importing: RFCParameter[] = [];
    const exporting: RFCParameter[] = [];
    const changing: RFCParameter[] = [];
    const tables: RFCTable[] = [];
    const exceptions: RFCException[] = [];
  ...
  private async parseParameters(parametersData: any[]): Promise<RFCParameter[]> {
  ...
  private parseParameterType(type: string): RFCParameterType {
    if (!type) return RFCParameterType.ELEMENTARY;
  ...
  private parseStructure(structureData: any): RFCStructure {
    return {
  ...
  private parseFields(fieldsData: any[]): RFCField[] {
  ...
  private async parseTables(tablesData: any[]): Promise<RFCTable[]> {
  ...
  private async parseExceptions(exceptionsData: any[]): Promise<RFCException[]> {
  ...
  private async parseDocumentation(docData: any): Promise<RFCDocumentation> {
    return {
  ...
  private parseParameterFromText(line: string): RFCParameter {
  ...
  private convertParameterToTable(param: RFCParameter): RFCTable {
    return {
      name: param.name,
      type: param.dataType,
      description: param.description,
  ...
  async validate(schema: SAPRFCSchema): Promise<SAPValidationResult> {
  ...
  getMetadata(schema: SAPRFCSchema): SAPMetadata {
    return {
  ...
  private countElements(content: SAPRFCContent): number {
    return (
      content.importing.length +
