...
import { SOAPAdapter, SAPSOAPSchema } from '../adapters/soap-adapter';
...
export class SAPValidator {
  ...
  async validateODataSchema(schema: SAPODataSchema): Promise<SAPValidationResult> {
    const errors: SAPValidationError[] = [];
    const warnings: SAPValidationWarning[] = [];
  ...
  async validateIDocSchema(schema: SAPIDocSchema): Promise<SAPValidationResult> {
    const errors: SAPValidationError[] = [];
    const warnings: SAPValidationWarning[] = [];
  ...
  async validateRFCSchema(schema: SAPRFCSchema): Promise<SAPValidationResult> {
    const errors: SAPValidationError[] = [];
    const warnings: SAPValidationWarning[] = [];
  ...
  async validateSOAPSchema(schema: SAPSOAPSchema): Promise<SAPValidationResult> {
    // SOAP 어댑터의 검증 로직 사용
    return this.soapAdapter.validate(schema);
  }

  // OData 검증 메서드들
  private validateODataVersion(
    schema: SAPODataSchema,
  ...
  private validateEntityContainer(
    schema: SAPODataSchema,
    errors: SAPValidationError[],
    warnings: SAPValidationWarning[]
  ): void {
    ...
    for (const association of schema.content.associations) {
      ...
      for (const end of association.ends) {
        const entityType = schema.content.entityTypes.find(
          et => `${schema.metadata.namespace}.${et.name}` === end.type
        );
        if (!entityType) {
          errors.push({
  ...
  private validateODataNamingConventions(
    schema: SAPODataSchema,
    errors: SAPValidationError[],
    warnings: SAPValidationWarning[]
  ): void {
  ...
  private validateODataPerformance(
    schema: SAPODataSchema,
    errors: SAPValidationError[],
    warnings: SAPValidationWarning[]
  ): void {
  ...
  // IDoc 검증 메서드들
  private validateIDocStructure(
    schema: SAPIDocSchema,
    errors: SAPValidationError[],
    warnings: SAPValidationWarning[]
  ): void {
  ...
  private validateIDocNamingConventions(
    schema: SAPIDocSchema,
    errors: SAPValidationError[],
    warnings: SAPValidationWarning[]
  ): void {
  ...
  private validateRFCNamingConventions(
    schema: SAPRFCSchema,
    errors: SAPValidationError[],
    warnings: SAPValidationWarning[]
  ): void {
  ...
  private validateRFCPerformance(
    schema: SAPRFCSchema,
    errors: SAPValidationError[],
    warnings: SAPValidationWarning[]
  ): void {
  ...
  private getTotalRulesCount(schemaType: string): number {
    switch (schemaType) {
      case 'OData':
        return 15;
      case 'IDoc':
