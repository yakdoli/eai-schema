...
export class ReportGenerator {
     ...
     */
    async generateReport(schema, validationResults, referenceResult, namespaceResult, typeResult) {
     ...
     */
    consolidateIssues(validationResults, referenceResult, namespaceResult, typeResult) {
        const allIssues = [];
        // 기본 검증 결과에서 이슈 수집
     ...
     */
    createReferenceIssues(referenceResult) {
        const issues = [];
        // 누락된 참조 이슈
     ...
     */
    createNamespaceIssues(namespaceResult) {
        const issues = [];
        // 네임스페이스 충돌 이슈
...
<body>
     ...
     * 이슈들의 HTML을 생성합니다
     */
    generateIssuesHTML(validationResults) {
        let html = '';
        validationResults.forEach(result => {
