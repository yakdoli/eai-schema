...
export class TypeValidator {
    ...
    findIncompatibleTypes(schema) {
            ...
            schema.content.entities.forEach((entity, entityIndex) => {
                if (entity.properties) {
                    entity.properties.forEach((property, propIndex) => {
                        if (property.type) {
                            const incompatibility = this.checkTypeCompatibility(property.type, property.constraints, {
        ...
        if (schema.content?.types) {
            schema.content.types.forEach((type, typeIndex) => {
                if (type.baseType) {
                    const incompatibility = this.checkInheritanceCompatibility(type.name, type.baseType, {
     ...
     * 타입 호환성을 검사합니다
     */
    checkTypeCompatibility(typeName, constraints, location) {
     ...
     * 상속 호환성을 검사합니다
     */
    checkInheritanceCompatibility(derivedType, baseType, location) {
     ...
     * 제약조건 호환성을 검사합니다
     */
    checkConstraintCompatibility(typeName, constraint) {
        const typeDef = this.typeDefinitions.get(typeName);
        if (!typeDef) {
     ...
     * 참조된 타입들을 수집합니다
     */
    collectReferencedTypes(schema, referencedTypes, typeReferences) {
        // 엔티티 속성에서 참조된 타입들
        if (schema.content?.entities) {
            schema.content.entities.forEach(entity => {
                if (entity.properties) {
     ...
     * 타입 참조를 추가합니다
     */
    addTypeReference(typeReferences, typeName, referencedBy) {
     ...
     * 제약조건을 검증합니다
     */
    validateConstraint(constraint, typeName) {
        if (!constraint.type) {
     ...
     * 순환 상속이 있는지 확인합니다
     */
    hasCircularInheritance(derivedType, baseType) {
