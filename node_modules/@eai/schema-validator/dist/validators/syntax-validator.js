import { SchemaType } from '@eai/shared';
import { ValidationSeverity, ValidationIssueType } from '../types';
/**
 * 구문적 정확성 검증기
 */
export class SyntaxValidator {
  ...
  *
     ...
     */
    async validateSchemaTypeSpecific(schema, issues) {
        switch (schema.type) {
     ...
     * WSDL 스키마 구문 검증
     */
    validateWSDLSyntax(schema, issues) {
        if (!schema.content?.operations || !Array.isArray(schema.content.operations)) {
            issues.push({
     ...
     * XSD 스키마 구문 검증
     */
    validateXSDSyntax(schema, issues) {
     ...
     * JSON Schema 구문 검증
     */
    validateJSONSchemaSyntax(schema, issues) {
        // JSON Schema 버전 검증
        if (!schema.version) {
            issues.push({
     ...
     * OpenAPI 스키마 구문 검증
     */
    validateOpenAPISyntax(schema, issues) {
        // OpenAPI 버전 검증
        if (!schema.version) {
            issues.push({
     ...
     * 필수 필드 검증
     */
    validateRequiredFields(schema, issues) {
        // 엔티티 필수 필드 검증
     ...
     * 데이터 타입 검증
     */
    validateDataTypes(schema, issues) {
        // 엔티티 속성 타입 검증
        if (schema.content?.entities) {
